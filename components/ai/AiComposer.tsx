  ...
  const sendPrompt = async (text: string) => {
    ...
    try {
      ...
      if (mindmap?.id) {
        ...
        // Check if operations include delete or update - if so, reload entire mindmap
        // to ensure UI reflects changes correctly
        const currentNodeCount = Array.isArray(nodes) ? nodes.length : 0
        const resultNodeCount = Array.isArray(result.nodes) ? result.nodes.length : 0
        const hasDeleteOrUpdate = result.nodes && result.edges && resultNodeCount < currentNodeCount

        if (hasDeleteOrUpdate) {
          // For delete/update operations, reload entire mindmap from backend
          // This ensures deleted nodes are removed and updated nodes reflect changes
          console.log('[AI Composer] Reloading mindmap after AI operation')
          if (mindmap?.id) {
            await loadMindmap(mindmap.id)
          }
        } else {
          // Original streaming logic for add-only operations
